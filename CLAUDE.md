...
├── 이미지풀.py             ← 카테고리별 큐레이션 풀 (2026-08-02 추가, 두 파일 공용)
│   로컬(images/pool/ self-host) + 원격(Unsplash hotlink) 항목을 합쳐 LRU로 고른다.
│   이전엔 풀 목록이 두 파일에 복붙돼 있어 한쪽만 고쳐지곤 했다 — 여기 한 곳에서만 관리한다.
│   `python3 이미지풀.py`로 카테고리별 장수 + cross-category 중복을 점검한다.
├── images/pool/           ← 자체 호스팅 풀 이미지 + manifest.json (링크가 죽지 않는다)
├── scripts/풀수집.py       ← 풀 후보 수집·선별 도구 (수집 → 대지 → 확정 3단계)
├── 이미지소스.py           ← 외부 이미지 소스 API (2026-08-02 추가, 두 파일 공용)
│   ※ 이미지필터.py·이미지소스.py는 **시그널코리아와 완전히 동일한 파일**이다 (2026-08-02 이식).
│     한쪽을 고치면 반대쪽에도 복사할 것 — `diff`로 동일함을 확인한다.
...
### [원인규명·수정됨] 헤드리스 무응답의 진짜 원인 = 작업 디렉터리 TCC (2026-08-06)
- **증상**: 08-05·08-06 로컬 발행이 `헤드리스 2회 모두 1200s 타임아웃(무응답)`. 출력 **한 줄 없음**, 세션 로그(`~/.claude/projects/**`)조차 안 생김. **시그널코리아도 같은 날부터 동일 증상** (두 채널 `llm_backend.py`는 바이트 단위 동일).
- **원인**: claude는 기동 직후 `getcwd()`→`open()`으로 cwd를 연다. cwd가 `~/Desktop` 아래면 macOS **TCC**가 동의를 요구하는데, launchd 백그라운드 잡에는 동의 창을 띄울 상대가 없어 `open()`이 **영원히 블록**된다. API 요청은 시작조차 못 한다.
- **방아쇠는 CLI 자동 업데이트**: 08-04 22:04 설치된 2.1.221 이후 첫 launchd 실행부터 두 채널 동시 사망. TCC 승인은 **바이너리 경로·서명 단위**인데 자동 업데이트는 `versions/<새버전>`에 새 바이너리를 깐다 → **바이너리에 권한을 주는 방식은 다음 업데이트에 또 깨진다.**
- **조치**: `llm_backend.py`에 `CLAUDE_CODE_CWD` 추가 — 헤드리스를 `$TMPDIR/aios-headless`에서 실행. 프롬프트가 자체 완결형이라 결과물 영향 없음. **Desktop 아래로 되돌리지 말 것.**
- **실측**(launchd·같은 바이너리·같은 환경, "1+1"): cwd=Desktop 아래 → 무한 대기(네트워크 연결 0건) / cwd=중립 → **13초 정상**.
- **덤**: 저장소 CLAUDE.md를 매 호출 컨텍스트에 안 싣게 되어 캐시 입력 22,073 → 6,404토큰.
- **아래 [완화됨] 항목(07-26~29)도 십중팔구 같은 원인이다** — "트리비얼 호출은 5초 만에 응답(대화형)"인데 launchd만 멈췄고, 07-27 수동 재시도가 성공한 것도 대화형이라 TCC를 통과했기 때문으로 설명된다. 재시도 완화책은 그대로 두되, 근본 원인은 이 항목으로 본다.
- **진단 요령**: 무응답을 만나면 **먼저 세션 로그가 생겼는지** 확인(`ls -lt ~/.claude/projects/<프로젝트>/`). 없으면 API에 닿기 전에 죽은 것 — 토큰·한도·프롬프트를 뒤질 필요가 없다. 멈춘 프로세스는 `sample <PID>`로 스택 확인.

### [수정됨] 사고 토큰이 32,000 출력 상한을 먹는다 (2026-08-06)
- **증상**: 헤드리스가 응답은 하는데 `KeyError: 'articles'`로 터진다. 출력이 32,000 상한에 걸리면 헤드리스가 턴을 이어 마저 쓰는데, `envelope["result"]`에는 **마지막 조각만** 담겨 앞이 잘린 JSON이 온다. `json_repair`가 그걸 그럴듯한 dict로 복구해버려 최상위 키가 사라진 채 통과한다.
- **원인**: `--effort medium`의 **사고(thinking) 토큰이 출력 예산을 함께 먹는다.** 시그널 3건 배치 실측 — medium: 사고 14,895자 + 출력 29,491토큰(상한 바로 밑, 한 번은 32,000+32,000+1,203으로 갈려 실패) / low: 사고 0자 + 출력 13,702토큰(여유 57%), 소요도 561초 → 247초.
- **조치**: `기사자동생성.py`의 생성 호출에 `effort="low"` 전달. **검수(`기사검수.py`)는 기본값 medium 유지** — 출력이 작아 상한 위험이 없고 판단 품질이 더 중요하다. `llm_backend.call_tool(..., effort=)` 파라미터로 호출별 지정.
- 산출물 품질 확인: 5단계 단락 수·속보형 빈 배열·현장 경험 문단·금지표현·카테고리 쿼터 전부 준수.
- **소재타임스는 5건을 한 호출로 뽑는다** — low 기준 추정 약 22,000토큰으로 상한 아래지만 시그널보다 여유가 적다. 프롬프트나 단락 수를 늘릴 땐 이 여유부터 확인할 것.
...
| 4 | 큐레이션 풀 | 불필요 | 카테고리 연관, 로컬 self-host + Unsplash hotlink 78장 |
...
### 풀 관리 원칙 (2026-08-02 개편)

풀은 **로컬 self-host + 원격 Unsplash hotlink** 두 갈래다. 총 78장 (카테고리당 18~20장).
...
**풀 늘리는 법** — `scripts/풀수집.py` 3단계. 최종 선별은 반드시 사람이 한다.
```bash
export PEXELS_API_KEY=... PIXABAY_API_KEY=...        # 루트 .env에 이미 있음
python3 scripts/풀수집.py 수집        # 카테고리별 후보 수집 → images/pool/_후보/
python3 scripts/풀수집.py 대지        # 번호 붙은 컨택트시트 생성 → 눈으로 고르기
python3 scripts/풀수집.py 확정 산업재 "7,10,22,39,40"   # 고른 번호만 편입
python3 이미지풀.py                   # 장수 + cross-category 중복 점검 (필수)
```
- 편입 후 **1280×720(16:9)로 리사이즈**한다. API 원본은 1900px대라 그대로 두면 저장소가 붓는다.
- `_후보/`는 `.gitignore` 대상 — 커밋하지 않는다.
- 카테고리당 최소 8장, 권장 15장 이상 (5기사/일 + LRU 간격 확보)
- **cross-category 중복 금지** — `이미지풀.validate()`가 감지한다. 실제로 편입 직후
  디스플레이 클린룸 사진 1장이 두 카테고리에 걸려 이 검증기에 잡혔다.
- **카테고리별 소재 공백 주의** — `pick()`은 카테고리만 보고 키워드는 안 본다.
  `산업재`에 전자재료·디스플레이 사진이 하나도 없어 OLED 기사에 자동차 공장이 나왔던 게 그 예다
  (2026-08-02 디스플레이 클린룸·배터리 공정·연구소 계열로 보강).
- **풀 건강 점검** — 풀 이미지 MD5가 `image_history.json`의 `hashes`에 다수 걸려 있으면
  위 '영구 해시 금지' 규칙이 깨진 것이다 (2026-08-02엔 33장 중 29장이 죽어 있었다).
